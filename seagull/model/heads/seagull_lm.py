...
class SeagullLM(Module):
    ...
    def forward(
        ...
        output_embeddings = seagull_output[0]
        if isinstance(output_embeddings, (list, tuple)):
            output_embeddings = output_embeddings[-1]

        if self.weight_tying:
            logits = output_embeddings @ self.seagull.embedding.token_embedding.weight.data.T
        else:
            logits = self.lm_head(output_embeddings)

        return logits, seagull_output
    @torch.no_grad()
    def talk(
        self,
        input_ids: torch.Tensor,
